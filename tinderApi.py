...
with open(u"./credentials.conf", "r") as crd:
    content = crd.readlines()
    if len(content) == 1:
...
def lastMatch():
    try:
        r = requests.get('https://api.gotinder.com/v2/fast-match/preview?locale=en', headers=getHeader())
        return r.content
    except requests.exceptions.RequestException as e:
        print("Something went wrong with getting last match:", e)
...
def get_recommendations():
    ...
    '''
    try:
        r = requests.get('https://api.gotinder.com/user/recs', headers=getHeader())
        return r.json()
    except requests.exceptions.RequestException as e:
...
def change_preferences(**kwargs):
    ...
    try:
        url = host + '/profile'
        r = requests.post(url, headers=getHeader(), data=json.dumps(kwargs))
        return r.json()
    except requests.exceptions.RequestException as e:
...
def update_location(lat, lon):
    ...
    try:
        ...
        r = requests.post(url, headers=getHeader(), data=json.dumps({"lat": lat, "lon": lon}))
        return r.json()
    except requests.exceptions.RequestException as e:
...
def set_webprofileusername(username):
    ...
    try:
        ...
        r = requests.put(url, headers=getHeader(),
...
def reset_webprofileusername(username):
    ...
    try:
        url = host + '/profile/username'
        r = requests.delete(url, headers=getHeader())
        return r.json()
    except requests.exceptions.RequestException as e:
...
def report(person_id, cause, explanation=''):
    ...
    try:
        ...
        r = requests.post(url, headers=getHeader(), data={
